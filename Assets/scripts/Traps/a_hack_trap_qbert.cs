...
public class a_hack_trap_qbert : a_hack_trap
{
    ...
    void AddColourToQbert(int colourToAdd)
    {
        ...
        if (!ColourFound)
        {
            ...
            Quest.SetVariable(101 + colourSequence.GetUpperBound(0), colourToAdd);
        }
    }
    ...
    /// If the current colour sequence is is not solved then the pyramid can be updated.
    void BeginQbertPyramid()
    {
        //Initalises the pyramid. 
        //Quest.variables[107] = 1;
        Quest.SetVariable(107, 1);
    }
    ...
    {
        int gamevar = Quest.GetVariable(108);
        tileX = gamevar & 0x3f;
    ...
    void setPreviousTileXY(int tileX, int tileY)
    {
        ...
        Quest.SetVariable(108, (tileY << 6) | (tileX));
    }
    ...
    int getTargetColour()
    {
        ...
        {
            if (Quest.GetVariable(i) != 0xFF)
            {
                target = Quest.GetVariable(i); //Quest.variables[i];
            }
            else
            {
                break;
    ...
    int[] getColourSequence()
    {
        ...
        for (int i = 101; i <= 105; i++)
        {
            if (Quest.GetVariable(i) != 0xFF)
            {
                Sequence[i - 101] = Quest.GetVariable(i);
            }
            else
            {
                break;
